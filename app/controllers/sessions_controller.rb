            ...
            if @session.save
                ...
                if params[:session][:join_session].to_i == 1
                    if !params[:session][:moderator_name].empty?
                        url = bbb.join_meeting_url(meeting_id, params[:session][:moderator_name],
                            options[:moderatorPW], joinViaHtml5: true)
                        redirect_to url
                    else
                        flash[:danger] = "The session was created but, couldn't be joined"\
                            "because no moderator name was provided"
